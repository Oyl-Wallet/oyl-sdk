...
export const chainMintStatus = new Command('chain-mint-status')
...
async function resumeSingleOrder(orderManager: ChainMintOrderManager, orderId: string, force: boolean = false, fallbackParams?: { feeRate?: number, childCount?: number }) {
  ...
  // 2. 检查恢复重试次数
  if (order.status === OrderStatus.RECOVERY_FAILED && !force) {
  ...
  // 4. 强制重试时重置订单状态
  if (force && (order.status === OrderStatus.RECOVERY_FAILED || 
  ...
  try {
    ...
    // 使用订单中保存的网络类型，不要做转换
    const wallet = new Wallet({ networkType: order.network as any })
    ...
    const wallets = await generateChainMintingWalletsFromEnv(
      provider.network, 
      order.relayWalletIndex
    )
    ...
    try {
      ...
      // 查找转移到中继地址的输出
      const relayOutput = parentTxDetails.vout.find((output: any) => 
        output.scriptpubkey_address === order.relayAddress
      )
    ...
    if (order.progress.completedChildTxs === 0) {
      ...
      const { performDryRunFeeCalculation } = await import('../alkanes/chainMinting')
      ...
      await executeChildTransactionChainWithResume(
        orderManager,
        order,
        wallets,
        provider,
        order.progress.parentTxId!,
        feeCalculation.relayFuelAmount,
        1, // 从第1笔子交易开始
        broadcastConfig,
        childCount
      )
 ...
 * 恢复所有中断的订单
 */
async function resumeAllInterruptedOrders(orderManager: ChainMintOrderManager, force: boolean = false, fallbackParams?: { feeRate?: number, childCount?: number }) {
  console.log(`\n🔄 恢复所有中断的订单`)
  console.log(`========================\n`)
 ...
 * 从子交易链中断点恢复
 */
async function resumeFromChildTransactionChain(
  ...
  try {
    ...
    await executeChildTransactionChainWithResume(
      ...
      wallets,
      provider,
      order.progress.lastTxId,
      order.progress.lastOutputAmount,
      startIndex,
      broadcastConfig,
      childCount
    )
 ...
 * 执行子交易链（支持从任意点开始）
 */
async function executeChildTransactionChainWithResume(
  orderManager: ChainMintOrderManager,
  order: any,
  wallets: any,
  provider: any,
