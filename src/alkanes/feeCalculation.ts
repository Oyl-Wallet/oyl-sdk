 ...
 * 构建模拟父交易用于费用计算
 */
async function buildMockParentTransaction({
  mainWallet,
  relayAddress,
  contractId,
  provider,
  mockRelayAmount = 100000 // 模拟中继金额
}: {
  mainWallet: { account: Account }
  relayAddress: string
  contractId: AlkaneContractId
  provider: Provider
  mockRelayAmount?: number
  ...
  const protostone = encodeProtostone({
    protocolTag: 1n,
    edicts: [],
    pointer: StandardVoutLayout.RELAY_OUTPUT,     // 成功：新mint的token发送到中继地址
    refundPointer: StandardVoutLayout.FINAL_CHANGE, // 失败：alkane资产退还到主钱包 (资金提供者)
    calldata: calldata
  })
 ...
 * 构建模拟子交易用于费用计算
 */
async function buildMockChildTransaction({
  relayWallet,
  contractId,
  provider,
  isLastTx = false,
  finalReceiverAddress = 'bc1qw508d6qejxtdg4y5r3zarvary0c5xw7kv8f3t4'
}: {
  relayWallet: { account: Account }
  contractId: AlkaneContractId
  provider: Provider
  isLastTx?: boolean
  ...
  const protostone = encodeProtostone({
    protocolTag: 1n,
    edicts: [],
    pointer: StandardVoutLayout.RELAY_OUTPUT,     // 成功：新mint的token + 传递的token发送到vout=0
    refundPointer: StandardVoutLayout.RELAY_OUTPUT, // 失败：传递的alkane资产退还到vout=0 (同一地址)
    calldata: calldata
  })
  ...
  const relayScript = bitcoin.script.compile([
    ...
    bitcoin.crypto.hash160(
      Buffer.from(relayWallet.account.nativeSegwit.pubkey, 'hex')
    )
  ])
 ...
 */
function calculatePsbtVSize(psbt: bitcoin.Psbt): number {
  try {
    // 尝试提取交易（如果PSBT完整）
 ...
 * 估算未完成PSBT的vSize
 */
function estimateTransactionVSize(psbt: bitcoin.Psbt): number {
...
export const HARDCODED_TRANSACTION_SIZES = {
  ...
  CHILD_TX_VSIZE: 146.25,
  
  /** 最后子交易vSize (24) - P2WPKH输入,P2TR输出,OP_RETURN */
  FINAL_CHILD_TX_VSIZE: 158.5
} as const
...
export async function performDryRunFeeCalculation({
  ...
  try {
    ...
    console.log(`   父交易vSize: ${HARDCODED_TRANSACTION_SIZES.PARENT_TX_VSIZE} vB`)
    console.log(`   普通子交易vSize (1-${childCount-1}): ${HARDCODED_TRANSACTION_SIZES.CHILD_TX_VSIZE} vB`)
    console.log(`   最后子交易vSize (${childCount}): ${HARDCODED_TRANSACTION_SIZES.FINAL_CHILD_TX_VSIZE} vB`)
    ...
    console.log(`💰 费用计算结果:`)
    console.log(`   父交易费用: ${parentTotalFee} sats (${HARDCODED_TRANSACTION_SIZES.PARENT_TX_VSIZE} vB × ${feeRate} sat/vB)`)
    console.log(`   普通子交易费用: ${normalChildFee} sats × ${normalChildCount} = ${normalChildTotalFees} sats`)
    console.log(`   最后子交易费用: ${finalChildFee} sats (${HARDCODED_TRANSACTION_SIZES.FINAL_CHILD_TX_VSIZE} vB × ${feeRate} sat/vB)`)
 ...
 * 构建真实父交易用于费用计算
 */
async function buildRealParentTransactionForFeeCalc({
  wallets,
  contractId,
  provider,
  utxos,
  estimatedRelayAmount
}: {
  wallets: ChainMintingWallets
  contractId: AlkaneContractId
  provider: Provider
  utxos: FormattedUtxo[]
  estimatedRelayAmount: number
}): Promise<bitcoin.Psbt> {
