 ...
 * 恢复中断的并行铸造执行
 */
export async function resumeParallelChainMinting(
  orderId: string,
  onProgress?: ParallelProgressCallback
): Promise<ParallelMintingResult> {
  ...
  try {
    ...
    // 1. 加载中断的订单
    const { ChainMintOrderManager, OrderExecutionMode, OrderStatus, SliceStatus } = await import('./chainMintOrder')
    ...
    if (order.executionMode !== OrderExecutionMode.SUPERCLUSTER) {
      throw new ChainMintingError(
        ChainMintingErrorType.INVALID_ADDRESS_TYPE,
        `订单不是Supercluster模式: ${order.executionMode}`
      )
    }
    ...
    const walletSystem = await generateMultiRelayWallets(
      network,
      order.executionParams.totalMints || 25
    )
    ...
    if (!order.progress.slices || order.progress.slices.length !== walletSystem.totalSlices) {
      throw new ChainMintingError(
        ChainMintingErrorType.INVALID_ADDRESS_TYPE,
        `分片信息不一致: 期望${walletSystem.totalSlices}, 订单中${order.progress.slices?.length || 0}`
      )
    }
    ...
    // 6. 分析可恢复的分片
    const recoverableSlices = order.progress.slices.filter(slice => 
    ...
    if (recoverableSlices.length === 0) {
      ...
      return {
        ...
        feeCalculation: {} as ParallelFeeCalculation,
        compositeParentTx: {
          transaction: { expectedTxId: compositeParentTxId } as BuiltTransaction,
          voutLayout: {} as CompositeParentVoutLayout
        },
        sliceResults: [],
        statistics: {
    ...
    const result: ParallelMintingResult = {
      ...
      totalDuration: endTime - startTime,
      walletSystem,
      feeCalculation: {} as ParallelFeeCalculation, // 恢复模式下不重新计算
      compositeParentTx: {
        transaction: { expectedTxId: compositeParentTxId } as BuiltTransaction,
        voutLayout: {} as CompositeParentVoutLayout
      },
 ...
 * 并行执行可恢复的分片
 */
async function executeRecoverableSlicesInParallel({
  orderId,
  orderManager,
  walletSystem,
  recoverableSlices,
  ...
  // 导入需要的枚举
  const { SliceStatus } = await import('./chainMintOrder')
  ...
  const sliceConfigs = recoverableSlices.map(sliceProgress => {
    ...
    const config = {
      ...
      feeCalculation: {
        ...
        feeDetails: {
          relayFuelAmount: 15000, // 估算值，实际执行时会重新计算
          childTx: { totalFee: 150 }
        }
      },
      finalReceiverAddress,
      provider: {} as Provider, // 需要从环境重新获取
      broadcastConfig,
  ...
  // 执行分片恢复（支持断点续传）
  const executeSliceWithRecovery = async (config: any) => {
    // 更新分片状态为执行中
 ...
 * 支持断点续传的分片执行
 */
async function executeSliceWithResume(
  config: any,
  ...
  if (config.resumeFromTxId && config.resumeFromStep > 0) {
    ...
    return executeSlice(modifiedConfig, onProgress)
  } else {
    // 正常从头执行
    return executeSlice(config, onProgress)
  }
}
 ...
 * 计算恢复执行统计
 */
function calculateRecoveryStatistics(
  allSliceResults: any[],
  startTime: number,
  endTime: number
): ParallelMintingResult['statistics'] {
 ...
 */
export async function executeParallelChainMintingWithTracking(
  config: ParallelMintingConfig,
  onProgress?: ParallelProgressCallback
): Promise<ParallelMintingResult> {
  ...
  try {
    ...
    // 1. 创建订单追踪
    const { ChainMintOrderManager, OrderExecutionMode, SliceStatus } = await import('./chainMintOrder')
    const orderManager = new ChainMintOrderManager()
    ...
    // 初始化分片状态到订单中
    console.log(`📋 保存分片配置...`)
    await initializeSliceConfigurations(
      orderManager,
      order.id,
      walletSystem,
      config
    ...
    // 更新分片费率配置
    await updateSliceConfigurations(
      orderManager,
      order.id,
      feeCalculation,
      feeRateConfig
 ...
 * 初始化分片配置到订单中
 */
async function initializeSliceConfigurations(
  orderManager: any,
  orderId: string,
  walletSystem: MultiRelayWalletSystem,
  config: ParallelMintingConfig
 ...
 * 更新分片费率配置
 */
async function updateSliceConfigurations(
  orderManager: any,
  orderId: string,
  feeCalculation: ParallelFeeCalculation,
  feeRateConfig: ParallelFeeRateConfig
  ...
  // 更新每个分片的费率配置
  const updatedSlices = order.progress.slices.map((slice: any, index: number) => {
 ...
 * 更新分片的vout映射信息
 */
async function updateSliceVoutMapping(
  orderManager: any,
  orderId: string,
  voutLayout: CompositeParentVoutLayout
): Promise<void> {
 ...
 * 执行所有分片并实时追踪每个分片的状态
 */
async function executeAllSlicesWithTracking({
  orderId,
  orderManager,
  walletSystem,
  feeCalculation,
  ...
  const sliceConfigs = feeCalculation.sliceCalculations.map((sliceCalc, index) => {
    ...
    const config = {
      ...
      finalReceiverAddress,
      provider,
      broadcastConfig: sliceProgress.originalConfig.broadcastConfig, // 使用保存的原始配置
  ...
  if (walletSystem.totalSlices === 1) {
    ...
    const result = await executeSliceWithTracking(
      orderManager,
      orderId,
      sliceConfigs[0],
      onProgress
    ...
    // 分片0立即开始（CPFP分片）
    console.log(`\n🚀 启动分片0 (CPFP分片)...`)
    const slice0Result = await executeSliceWithTracking(
      orderManager,
      orderId,
      sliceConfigs[0],
      onProgress
    ...
    while (!confirmed) {
      try {
        const acceptanceResult = await waitForTransactionAcceptance(
          compositeParentTx.expectedTxId,
          provider,
          0 // 无超时，无限等待
        )
    ...
    if (sliceConfigs.length > 1) {
      ...
      const remainingResults = await executeRemainingSlicesWithTracking(
        orderManager,
        orderId,
        remainingConfigs,
        enableParallel,
 ...
 * 执行单个分片并追踪状态
 */
async function executeSliceWithTracking(
  orderManager: any,
  orderId: string,
  config: any,
  onProgress?: (sliceIndex: number, progress: { currentStep: number, totalSteps: number, message: string }) => void
 ...
 * 执行剩余分片（并行或串行）
 */
async function executeRemainingSlicesWithTracking(
  orderManager: any,
  orderId: string,
  sliceConfigs: any[],
  enableParallel: boolean,
  ...
  if (!enableParallel) {
    ...
    for (let i = 0; i < sliceConfigs.length; i++) {
      ...
      // 启动分片执行
      const slicePromise = executeSliceWithTracking(
        orderManager,
        orderId,
        config,
        onProgress
