...
class RBFAccelerationTester {
  ...
  private mockTransactions: Map<string, MockTransactionInfo> = new Map()
  ...
  private createMockTransactions(): void {
    const mockTxs: MockTransactionInfo[] = [
      {
        txId: 'tx1_low_fee',
        feeRate: 2,
      ...
      },
      {
        ...
        lastAccelerationTime: Date.now() - 400000, // 6.7 minutes ago
        confirmed: false
      },
      {
        ...
        feeRate: 3,
        accelerationAttempts: 1,
        lastAccelerationTime: Date.now() - 120000, // 2 minutes ago
        confirmed: false
      },
      {
        ...
        lastAccelerationTime: Date.now() - 600000, // 10 minutes ago
        confirmed: false
      },
      {
        txId: 'tx5_confirmed',
  ...
  private shouldAccelerate(tx: MockTransactionInfo, currentMedianFeeRate: number): boolean {
    // 基本条件检查
    if (tx.confirmed) {
  ...
  private calculateNewFeeRate(currentFeeRate: number, currentMedianFeeRate: number): number {
    const newFeeRate = Math.min(
      Math.ceil(currentMedianFeeRate * this.config.accelerateFeeMultiplier),
      currentFeeRate + this.config.maxFeeIncrease,
  ...
  private simulateAcceleration(tx: MockTransactionInfo, newFeeRate: number): boolean {
  ...
  public testAccelerationLogic(currentMedianFeeRate: number): void {
  ...
  public testDifferentFeeRateScenarios(): void {
