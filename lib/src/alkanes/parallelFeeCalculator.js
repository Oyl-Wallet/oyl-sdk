...
function calculateCompositeParentFee(totalSlices, feeRate) {
    ...
    // 每个额外分片增加33字节
    const totalVSize = (0, feeCalculation_1.calculateParentTxVSize)(totalSlices);
 ...
 */
function calculateSliceMintCount(totalMints, sliceIndex) {
 ...
 * 计算并行执行的总体统计
 */
function calculateParallelSummary(compositeParentFee, sliceCalculations) {
...
function compareParallelVsSerialFees(parallelFees, serialFeeRate) {
    ...
    const singleExecutionFee = Math.ceil((0, feeCalculation_1.calculateParentTxVSize)(1) * serialFeeRate) +
