...
exports.smartBroadcastTransactionChainWithRpc = exports.broadcastTransactionChainWithRpc = exports.generateBroadcastSummary = exports.formatBatchBroadcastResult = exports.monitorTransactionChainStatus = exports.smartBroadcastTransactionChain = exports.broadcastChildTransactionsInParallel = exports.broadcastTransactionChain = exports.broadcastTransactionChainParallel = exports.waitForTransactionAcceptance = exports.broadcastSingleTransaction = exports.broadcastSingleTransactionWithRpc = void 0;
 ...
 */
async function broadcastChildTransactionsInParallel({ childTransactions, provider, config = chainMinting_1.DEFAULT_BROADCAST_CONFIG, batchSize = 3 // 每批并行广播的数量
 }) {
    ...
    for (let i = 0; i < childTransactions.length; i += batchSize) {
        ...
        // 并行广播当前批次
        const batchPromises = batch.map(tx => broadcastSingleTransaction(tx.psbtHex, tx.expectedTxId, provider, config));
...
exports.broadcastChildTransactionsInParallel = broadcastChildTransactionsInParallel;
/**
 * 智能广播策略
 *
 * 根据网络状况和交易依赖关系自动选择最佳广播策略
 */
async function smartBroadcastTransactionChain({ parentTransaction, childTransactions, provider, config = chainMinting_1.DEFAULT_BROADCAST_CONFIG }) {
    ...
    if (networkAnalysis.congestionLevel === 'low' && childTransactions.length <= 10) {
        ...
        return broadcastTransactionChain({
            ...
            config: {
                ...config,
                retryDelayMs: config.retryDelayMs * 2,
                confirmationTimeoutMs: config.confirmationTimeoutMs * 1.5 // 增加确认超时
            }
        });
    }
}
exports.smartBroadcastTransactionChain = smartBroadcastTransactionChain;
// ============================================================================
// 辅助工具函数
// ============================================================================
/**
 ...
 * 分析网络状况（简化版本）
 */
async function analyzeNetworkConditions(_provider) {
    try {
        // 简化的网络分析 - 使用费用估算API
 ...
 * 监控交易链的广播状态
 */
async function monitorTransactionChainStatus({ parentTxId, childTxIds, provider, pollIntervalMs = 10000, // 每10秒检查一次
maxMonitoringTimeMs = 300000 // 最多监控5分钟
    ...
    while (Date.now() - startTime < maxMonitoringTimeMs) {
        try {
            ...
            // 检查所有子交易状态
            const childStatuses = await Promise.all(childTxIds.map(async (txId) => {
                try {
...
exports.monitorTransactionChainStatus = monitorTransactionChainStatus;
// ============================================================================
// 格式化和报告功能
// ============================================================================
/**
 * 格式化批量广播结果
 */
function formatBatchBroadcastResult(result) {
...
exports.formatBatchBroadcastResult = formatBatchBroadcastResult;
/**
 * 生成广播摘要报告
 */
function generateBroadcastSummary(result) {
    return {
        summary: {
...
exports.generateBroadcastSummary = generateBroadcastSummary;
// ============================================================================
// 自定义RPC广播功能
// ============================================================================
/**
 ...
 * 智能广播交易链 - 自动选择最佳RPC
 */
async function smartBroadcastTransactionChainWithRpc({ parentTransaction, childTransactions, networkType, config = chainMinting_1.DEFAULT_BROADCAST_CONFIG }) {
    ...
    try {
        ...
        throw new chainMinting_1.ChainMintingError(chainMinting_1.ChainMintingErrorType.BROADCAST_ERROR, `智能广播失败: ${error.message}`, { error: error.message });
    }
}
exports.smartBroadcastTransactionChainWithRpc = smartBroadcastTransactionChainWithRpc;
// ============================================================================
// 工具函数已在上方定义
// ============================================================================
