...
const createExecutePsbt = async ({ alkanesUtxos, frontendFee, feeAddress, utxos, account, protostone, provider, feeRate, fee = 0, alkaneReceiverAddress, }) => {
    try {
...
const execute = async ({ alkanesUtxos, utxos, account, protostone, provider, feeRate, signer, frontendFee, feeAddress, alkaneReceiverAddress, }) => {
    const { fee } = await (0, exports.actualExecuteFee)({
        alkanesUtxos,
        frontendFee,
        feeAddress,
