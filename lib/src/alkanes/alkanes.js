...
exports.batchExecute = exports.estimateExecuteFeeWithoutChange = exports.toTxId = exports.createTransactReveal = exports.execute = exports.executePsbt = exports.actualExecuteFee = exports.actualTransactRevealFee = exports.deployReveal = exports.createDeployRevealPsbt = exports.deployCommit = exports.createDeployCommitPsbt = exports.createExecutePsbt = exports.encodeProtostone = void 0;
...
const createExecutePsbt = async ({ alkanesUtxos, frontendFee, feeAddress, utxos, account, protostone, provider, feeRate, fee = 0, alkaneReceiverAddress, enableRBF = false, }) => {
    try {
        ...
        const spendTargets = utils_1.inscriptionSats + Number(feeSatEffective);
...
async function addInputForUtxo(psbt, utxo, account, provider, enableRBF = false) {
...
const createDeployCommitPsbt = async ({ payload, utxos, tweakedPublicKey, account, provider, feeRate, fee, }) => {
    try {
        ...
        psbt.addOutput({
            value: finalFee + wasmDeploySize + utils_1.inscriptionSats,
...
const createDeployRevealPsbt = async ({ protostone, receiverAddress, script, feeRate, tweakedPublicKey, provider, fee = 0, commitTxId, }) => {
    try {
        ...
        if (revealTxChange > utils_1.inscriptionSats) {
            psbt.addOutput({
...
const actualExecuteFee = async ({ alkanesUtxos, utxos, account, protostone, provider, feeRate, frontendFee, feeAddress, alkaneReceiverAddress, }) => {
...
const executePsbt = async ({ alkanesUtxos, utxos, account, protostone, provider, feeRate, frontendFee, feeAddress, alkaneReceiverAddress, }) => {
...
const execute = async ({ alkanesUtxos, utxos, account, protostone, provider, feeRate, signer, frontendFee, feeAddress, alkaneReceiverAddress, enableRBF = false, }) => {
    const { fee } = await (0, exports.actualExecuteFee)({
        alkanesUtxos,
        frontendFee,
        feeAddress,
...
const estimateExecuteFeeWithoutChange = async ({ feeRate, frontendFee, inputCount = 1, }) => {
    const MIN_RELAY = 546n;
    const feeSatEffective = frontendFee && frontendFee >= MIN_RELAY ? frontendFee : 0n;
    ...
    const estimatedTxSize = (0, btc_1.minimumFee)({
    ...
    const totalRequired = utils_1.inscriptionSats + Number(feeSatEffective) + estimatedFee;
    return {
        estimatedFee,
        totalRequired,
        breakdown: {
...
exports.estimateExecuteFeeWithoutChange = estimateExecuteFeeWithoutChange;
const batchExecute = async ({ alkanesUtxos, utxos, account, protostone, provider, feeRate, signer, frontendFee, feeAddress, accountCount, mnemonic, alkaneReceiverAddress, }) => {
    try {
        if (accountCount < 1) {
            throw new Error('Account count must be at least 1');
        }
        // Generate child accounts and their corresponding signers
        const accountsWithSigners = [
            { account, signer }
        ];
        for (let i = 1; i < accountCount; i++) {
            const childAccount = (0, __1.mnemonicToAccount)({
                mnemonic,
                opts: {
            ...
            const childPrivateKeys = (0, __1.getWalletPrivateKeys)({
                mnemonic,
                opts: {
                    network: account.network,
                    index: i,
            ...
            const childSigner = new __1.Signer(account.network, {
        ...
        const executePromises = accountsWithSigners.map(async ({ account: acc, signer: accSigner }, index) => {
            try {
                ...
                const { accountUtxos: accUtxos } = await (0, utxo_1.accountUtxos)({
                    account: acc,
                    provider,
                });
