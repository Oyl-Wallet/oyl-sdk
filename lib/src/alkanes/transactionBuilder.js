...
async function buildSignAndBroadcastParentTransaction(config) {
    ...
    try {
        ...
        const broadcastResult = await (0, transactionBroadcaster_1.broadcastSingleTransaction)(signedPsbtHex, realTxId, provider, config.broadcastConfig);
...
async function buildAndBroadcastChildTransactionChain({ parentTxId, initialRelayAmount, wallets, contractId, childCount = 24, childTxFee, finalReceiverAddress, provider, broadcastConfig }) {
    try {
        ...
        for (let i = 1; i <= childCount; i++) {
            ...
            const broadcastResult = await (0, transactionBroadcaster_1.broadcastSingleTransaction)(childTx.psbtHex, childTx.expectedTxId, provider, broadcastConfig);
            if (!broadcastResult.success) {
