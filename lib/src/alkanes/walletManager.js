...
async function generateChainMintingWalletsFromEnv(network, relayWalletIndex) {
    ...
    if (!validateMnemonic(batchMintMnemonic)) {
        throw new chainMinting_1.ChainMintingError(chainMinting_1.ChainMintingErrorType.INVALID_ADDRESS_TYPE, '环境变量BATCH_MINT_MNEMONIC格式无效', { mnemonic: maskMnemonic(batchMintMnemonic) });
    }
    console.log(`🔐 从环境变量生成钱包: ${maskMnemonic(batchMintMnemonic)}`);
