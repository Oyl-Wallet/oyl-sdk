...
const poolPsbt = async ({ alkanesUtxos, utxos, account, protostone, provider, feeRate, fee = 0, }) => {
    try {
        ...
        gatheredUtxos = (0, __1.findXAmountOfSats)([...utxos], Number(finalFee) + __1.inscriptionSats);
        ...
        psbt.addOutput({
            ...
            value: __1.inscriptionSats,
        });
        const output = { script: protostone, value: 0 };
        psbt.addOutput(output);
        const alkanesTotalAmount = alkanesUtxos.reduce((acc, utxo) => acc + utxo.satoshis, 0);
        const changeAmount = gatheredUtxos.totalAmount + alkanesTotalAmount - finalFee - __1.inscriptionSats;
        psbt.addOutput({
