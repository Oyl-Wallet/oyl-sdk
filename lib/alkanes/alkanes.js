...
const createExecutePsbt = async ({ alkanesUtxos, frontendFee, feeAddress, utxos, account, protostone, provider, feeRate, fee = 0, enableRbf = false, }) => {
    try {
...
async function addInputForUtxo(psbt, utxo, account, provider, enableRbf) {
    ...
    switch (type) {
        case 0: {
            ...
            psbt.addInput({
                ...
                ...(enableRbf && { sequence: constants_1.RBF_SEQUENCE }),
            });
            break;
        }
        case 2: {
...
const executePsbt = async ({ alkanesUtxos, utxos, account, protostone, provider, feeRate, frontendFee, feeAddress, enableRbf = false, }) => {
...
const execute = async ({ alkanesUtxos, utxos, account, protostone, provider, feeRate, signer, frontendFee, feeAddress, enableRbf = false, }) => {
    const { fee } = await (0, exports.actualExecuteFee)({
        alkanesUtxos,
        frontendFee,
        feeAddress,
