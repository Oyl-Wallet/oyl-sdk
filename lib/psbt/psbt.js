...
const getEstimatedFee = async ({ feeRate, psbt, provider, }) => {
    ...
    const outputSizes = psbtObj.txOutputs.map((output) => {
        ...
        if (output.script[0] === 0x6a) {
            ...
            // For OP_RETURN, we only count the script length
            // The 9 bytes base size is already included in the script length
    ...
    const baseSize = BASE_OVERHEAD +
        inputVarIntSize +
        outputVarIntSize +
        psbtObj.data.inputs.reduce((sum, input) => sum + SIZES[detectInputType(input)].input.unsigned, 0) +
        totalOutputSize;
    // Calculate witness size
    const witnessSize = psbtObj.data.inputs.reduce((sum, input) => sum + SIZES[detectInputType(input)].input.getWitnessSize(input), 0);
    // Calculate vsize using the formula: (base_size * 3 + total_size) / 4
    const totalSize = baseSize + witnessSize;
    const vsize = Math.round((baseSize * 3 + totalSize) / 4);
